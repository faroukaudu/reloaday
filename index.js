...
app.get("/super-Admin", async (req, res) => {
  ...
  if (!req.isAuthenticated()) {
    ...
    try {
      ...
      const profitLoss = await Activity.aggregate([
        {
          $group: {
            _id: null,
      ...
      const profitLossAnynomous = await Anonymous.aggregate([
        {
          $match: { successful: true }   // only successful transactions
        },
        {
          $group: {
            _id: null,
      ...
      const total1 = profitLoss[0]?.total || 0;
      const total2 = profitLossAnynomous[0]?.total || 0;
